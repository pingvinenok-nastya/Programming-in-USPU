	...
	std::vector<std::vector<char>> track;
		...
		while (p < track[trk].size()) {
			...
			case 0x90 ... 0x9f:{
				...
				break;
			case 0xa0 ... 0xaf:
				std::cout << " Это метасобытие3 - тут скорее всего пропуск" << std::endl;
				break;
			case 0xb0 ... 0xbf:
				int num_par, par;
			...
			//здесь может быть массив с инструментами 128 штук
				p++;
				break;
			case 0xd0 ... 0xdf:
				std::cout << "Aftertouch" ;
			...
			case 0xff:
			{
				...
				switch(meta_type) {
					...
					case 0x03:
					  {
						...
						std::string str;
						while (len--) str += char(track[trk][p++]);
						std::cout << "Эта дорожка называется: " << str << std::endl;
					  }
					  break;
					case 0x04:
					...
					case 0x58:
						...
						break;
					default:
					  std::cout << "А вот не знаю я мету с номером " << meta_type << std::endl;
					}
			}
				break;
			default:
